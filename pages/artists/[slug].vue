  ...
  <div class="container py-10 md:py-12 lg:py-14">
    ...
    <div v-if="loading" class="animate-pulse space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start">
        ...
        <div class="skeleton-img h-[360px] md:h-[420px]"></div>
      </div>
    </div>
    ...
    <template v-else>
      ...
      <div v-if="artist">
        <!-- Top section -->
        <div
          class="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(260px,360px)] gap-8 lg:gap-12 items-start mb-12 md:mb-14">
          <!-- Left -->
          ...
          <!-- Right: portrait -->
          <div class="order-1 lg:order-2 img-frame overflow-hidden bg-transparent">
            <img v-if="artist.imageUrl" :src="artist.imageUrl" :alt="artist.fullName"
              class="w-full max-h-[420px] object-contain" />
            <div v-else
              class="h-[320px] md:h-[380px] flex items-center justify-center text-[var(--color-text-muted)]/40">
          ...
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-7">
            <div v-for="w in artworks" :key="w.id" class="art-card overflow-hidden">
              ...
              <div class="p-4 space-y-1.5">
                <div class="font-serif text-[1.12rem] text-[var(--color-text)] leading-tight">
                  {{ w.title }}
                </div>

                <div v-if="w.year || w.description" class="text-[12px] leading-snug text-[var(--color-text-muted)]">
                  <span v-if="w.year">{{ w.year }}</span>
                  <span v-if="w.year && w.description"> · </span>
                  <span v-if="w.description">{{ w.description }}</span>
                </div>

                <div v-if="exMap[w.exhibitionId]" class="text-[12px] leading-snug text-[var(--color-text-muted)] pt-1">
                  з експозиції:
