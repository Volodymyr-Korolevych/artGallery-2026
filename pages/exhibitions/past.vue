...
const fetchAll = async () => {
  loading.value = true
  const { data: ex } = await supabase.from('exhibitions').select('id,slug,title,status,"startDate","endDate","cardUrl","painterId","isPublished"').eq('isPublished',true).eq('status','past').order('startDate',{ascending:false})
...
<template>
  <div>
    ...
    <h1 class="font-serif text-5xl font-semibold text-neutral-900 mb-10">Минулі експозиції</h1>

    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      <div v-for="n in 6" :key="n" class="animate-pulse">
        <div class="bg-neutral-200 aspect-[3/2] rounded mb-3"></div>
        <div class="h-3 bg-neutral-100 rounded w-1/2 mb-2"></div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
      ...
      />
      <div v-if="!items.length" class="col-span-3 text-sm text-neutral-400 italic">Немає експозицій для показу.</div>
    </div>
  </div>
</template>
