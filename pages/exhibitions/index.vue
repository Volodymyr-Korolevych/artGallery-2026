  ...
  <div class="container py-10 md:py-12 lg:py-14">
    <div class="max-w-3xl mb-8 md:mb-10">
      ...
      <p class="text-sm md:text-base text-[var(--color-text-muted)] leading-relaxed">
        Перегляньте поточні, майбутні та архівні експозиції галереї.
      </p>
    </div>

    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-7">
      <div v-for="n in 6" :key="n" class="art-card overflow-hidden">
    ...
    <div v-else-if="items.length" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-7">
      <ExhibitionCard v-for="e in items" :key="e.id" :slug="e.slug" :title="e.title" :status="e.status"
        :startDate="e.startDate" :endDate="e.endDate" :cardUrl="e.cardUrl" :artist="artistOf(e)" />
    </div>

    <div v-else class="max-w-xl">
      <div class="alert-info">
        Наразі немає виставок для показу.
      </div>
    </div>
  </div>
</template>