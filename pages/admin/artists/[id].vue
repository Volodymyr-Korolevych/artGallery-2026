    ...
    <div class="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_300px] gap-6 items-start">
      ...
      <div>
        <div class="art-card p-5 space-y-4">
          ...
          <div class="img-frame h-36 flex items-center justify-center bg-transparent">
            <img v-if="form.imageUrl" :src="form.imageUrl" class="h-full w-full object-contain" />
            <div v-else
              class="h-full w-full flex items-center justify-center font-serif text-4xl text-[var(--color-text-muted)]/40">
              {{ form.fullName?.[0] }}
            </div>
          </div>

          <div>
            <button @click="pickFile" class="btn-outline">
              Оберіть файл
            </button>
